...
6. Keep observing at least every 30 seconds with a bounded wait. The main conversation or a dedicated monitoring subagent may judge progress from advancing scan counters, fresh SSE/log/tool-call records, file changes, active process work, worker phase transitions, incomplete model output, or other task-specific signals. A timer tick or an unchanged “thinking” label alone is not progress.
7. On expiry, treat the incident as a mandatory review request. Inspect actual outputs and the live task state. If evidence shows normal or opaque ongoing work, send `heartbeat TAG --note EVIDENCE` and continue; the heartbeat restores a provisionally stalled job to `armed`. No completed command, unchanged files, no child process, or an absence-only log gap does not prove a stall and never authorizes interruption. Stop or disarm only after positive terminal/failure evidence or explicit user instruction. An incident never authorizes automatic replay, quota spending, prompt sending, or destructive cleanup.

Automatic log-state alerts (`post_tool_seconds`, `response_seconds`, `critical_seconds`, `opaque_model_seconds`, `stream_silence_seconds`, `tool_warning_seconds`, and `tool_critical_seconds`) are independent detector settings. They notify and record; they do not kill a task. Opaque model work defaults to longer review windows because code/patch construction can be silent at completed-event granularity. Manual per-attempt classification controls only explicitly armed jobs.
